...
{
    /// <summary>
    /// ReadJEnc ファイル読み出し＆ファイル文字コード種類自動判別(Rev.20140818)
    /// </summary>
    public class FileReader : IDisposable
    {   ////////////////////////////////////////////////////////////////////////
        // <FileReader.cs> ReadJEnc File読出＆文字コード自動判別(Rev.20140818)
        ...
        public virtual CharCode Read(FileInfo File)
        {
            ...
            {   //無用なDiskIOを極力行わないよう、オープン前にもファイルサイズチェック
                ...
                {   //オープン後の実際のファイルサイズによるチェック
                    ...
                    if (filesize > 65536)
                    {   //一定サイズ以上の大きいファイルなら、BOM/マジックナンバー判定に必要な先頭バイトを読み込み判断
        ...
        protected virtual CharCode GetPreamble(long Length)
        {
            ...
            if (ret != null) { return ret; } //■BOMつきUTF確定
            if (Array.IndexOf<byte>(this.Bytes, 0x00, 0, this.Length) >= 0)
            {   //テキストファイルと判定できず＆ファイル先頭にバイナリファイル特徴の0x00が登場している
                if (ReadJEnc.SeemsUTF16N(this.Bytes, (int)Length) != null)
                {   //UTF16Nの可能性があれば保留
                    return null;
                }
                return FileType.GetBinaryType(this.Bytes, this.Length); //■バイナリ確定（マジックナンバーからファイル種類を決定）
            }
            //決められなかったらnullを返す
            return null;
        }
